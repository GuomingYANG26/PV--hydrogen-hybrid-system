# -*- coding: utf-8 -*-
"""
Created on Tue Jul 12 20:25:51 2022
author: Guoming Yang
...
def lambda_air_function (lambda_air1, lambda_air2):
    lambda_air1 = lambda_air1
    lambda_air2 = lambda_air2
    # --------------------------------construct model------------------------------------
    ...
    #photo_hydrogen.addConstr((X3==0), name='capacity of electrolyzer under conventional case') 
    ...
    (mu_co*delta_co + mu_nox1*delta_nox + mu_hc*delta_hc)*110*0.73/39
    ...
    return obj, X2.x, X3.x, R1.x, R2.x, R3.x
    #return obj, lambda_air1, lambda_air2, X2.x, X3.x
    #return obj, lambda_air1, lambda_air2, X2.x, X3.x

lambda_air1 = np.linspace(0,20,21)      #the support level for PV from government
lambda_air2 = np.linspace(0,20,21)      #the support level for hydrogen from government
df0 = pd.DataFrame()                    #save objective function value
df1 = pd.DataFrame()                    #save the rated capacity of transformer
df2 = pd.DataFrame()                    #save the rated power of electrolyzers
df3 = pd.DataFrame()                    #save annual hydrogen sale revenue
df4 = pd.DataFrame()                    #save annual electricity sale revnue
df5 = pd.DataFrame()                    #save annual environmental benefits

for m in range(21):
    for n in range(21):
        print('-----------------------------------------------------------------')
        print('({0},{1})'.format(m,n))
        df0.loc[m, n], df1.loc[m, n], df2.loc[m, n], df3.loc[m, n],df4.loc[m, n],df5.loc[m, n] = lambda_air_function (lambda_air1[m], lambda_air2[n])
